...
CONF_LFS_SAM_READS_         = "";
CONF_LFS_OUT_               = "";
CONF_CORREL_STAT_SPEARMAN_  = False; # If True (default False), use Spearman rank correlation instead of Pearson correlation

REPORT_HTML                 = "Hercules-report.html";
REPORT_TXT                  = "All-fourmers.txt";
REPORT_HTML_SPEARMAN        = "Hercules-report-spearman.html";
REPORT_TXT_SPEARMAN         = "All-fourmers-spearman.txt";
_STR_CORREL_ 		    = "Pearson Correlation";
_STR_DATE_TIME_		    = datetime.date.today().strftime("%B %d, %Y");
...
def printCorrelStat():
	...
	lstHTMLReport.append("<h2>Fourmer Outliers ({correl})</h2>".format(correl=_STR_CORREL_));
...
<h2>All fourmer motifs ({correl})</h2>""".format(dt=_STR_DATE_TIME_, gtf=CONF_LFS_GTF_FILE_FILTERED_, sam=CONF_LFS_SAM_READS_, correl=_STR_CORREL_);
...
Hercules-MPI motif analysis report

{dt}
GTF Annotation: {gtf}
SAM reads: {sam}

All fourmer motifs ({correl})
""".format(dt=_STR_DATE_TIME_, gtf=CONF_LFS_GTF_FILE_FILTERED_, sam=CONF_LFS_SAM_READS_, correl=_STR_CORREL_));
	...
	if CONF_CORREL_STAT_SPEARMAN_:
		saveText(CONF_LFS_OUT_ + REPORT_TXT_SPEARMAN, lstReportAllFourmers);
	else:
	...
	if CONF_CORREL_STAT_SPEARMAN_:
		saveText(CONF_LFS_OUT_  + REPORT_HTML_SPEARMAN, lstHTMLReport);
	else:
...
parser.add_option("-r", "--spearman", action="store_true", dest="SPEARMAN", default=False);
...
if len(sys.argv) == 1:
    ...
    CONF_CORREL_STAT_SPEARMAN_		= options.SPEARMAN;

    if CONF_CORREL_STAT_SPEARMAN_:
        _STR_CORREL_ = "Spearman's Rank Correlation";
    else:
        _STR_CORREL_ = "Pearson Correlation";
